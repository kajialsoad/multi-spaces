      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            ListTile(
              ...
              onTap: () async {
                ...
                try {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('Enforcing fresh login...'),
                      backgroundColor: Colors.orange,
                      duration: Duration(seconds: 1),
                    ),
                  );
                  final ok = await AppService.enforceFreshLoginById(app.clonedAppId!);
                  if (!mounted) return;
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(ok
                          ? 'Fresh login enforced. Next launch will require sign-in.'
                          : 'Failed to enforce fresh login'),
            ...
            if (app.isCloned && app.clonedAppId != null)
              ListTile(
                leading: const Icon(Icons.cleaning_services, color: Colors.orange),
                title: const Text('Clear Web/Cookie Data (this clone)', style: TextStyle(color: Colors.white)),
                subtitle: const Text('Cookies, WebView storage, auth caches', style: TextStyle(color: Colors.white70, fontSize: 12)),
                onTap: () async {
                  Navigator.pop(context);
                  try {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('Clearing web data for this clone...'),
                        backgroundColor: Colors.orange,
                        duration: Duration(seconds: 1),
                      ),
                    );
                    final ok = await AppService.clearCloneWebArtifactsById(app.clonedAppId!);
                    if (!mounted) return;
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(ok
                            ? 'Web data cleared for this clone'
                            : 'Failed to clear web data'),
                        backgroundColor: ok ? Colors.green : Colors.red,
                        duration: const Duration(seconds: 2),
                      ),
                    );
            ...
            ListTile(
              ...
              title: const Text('Clear Global Web Data', style: TextStyle(color: Colors.white)),
              subtitle: const Text('Affects all apps and clones', style: TextStyle(color: Colors.white70, fontSize: 12)),
              onTap: () async {
                Navigator.pop(context);
                try {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('Clearing global web data...'),
                      backgroundColor: Colors.orange,
                      duration: Duration(seconds: 1),
                    ),
                  );
                  final ok = await AppService.clearGlobalWebArtifacts();
                  if (!mounted) return;
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(ok
                          ? 'Global web data cleared successfully'
                          : 'Failed to clear global web data'),
                      backgroundColor: ok ? Colors.green : Colors.red,
                      duration: const Duration(seconds: 2),
                    ),
                  );
