...
class MainActivity: FlutterActivity() {
    ...
    companion object {
        ...
        private const val ACTION_DELETE_CLONE = "com.multispace.app.ACTION_DELETE_CLONE"
        private const val EXTRA_CLONED_APP_ID = "cloned_app_id"
        private const val EXTRA_CLONED_PACKAGE = "cloned_package_name"
    }
    
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        try {
            handleDeleteCloneIntent(intent)
        } catch (e: Exception) {
            Log.e(TAG, "Error handling startup intent", e)
        }
    }
    ...
    private fun handleDeleteCloneIntent(intent: Intent?) {
        if (intent == null) return
        if (intent.action != ACTION_DELETE_CLONE) return

        val extras = intent.extras
        Log.d(TAG, "Received ACTION_DELETE_CLONE with extras=$extras")

        // Ensure singletons are initialized if configureFlutterEngine hasn't run yet
        if (!::databaseHelper.isInitialized) {
            databaseHelper = DatabaseHelper.getInstance(this)
        }
        if (!::virtualSpaceEngine.isInitialized) {
        ...
        backgroundExecutor.execute {
            try {
                val clonedAppId = resolveClonedAppIdFromIntent(intent)
                if (clonedAppId == null) {
                    Log.w(TAG, "ACTION_DELETE_CLONE: No valid cloned app ID resolved from intent")
                    return@execute
                }
                performCloneRemoval(clonedAppId)
            } catch (e: Exception) {
                Log.e(TAG, "ACTION_DELETE_CLONE processing failed", e)
            }
        }
    }

    private fun resolveClonedAppIdFromIntent(intent: Intent): Long? {
        val extras = intent.extras
        // Try direct ID first (robustly handling different types)
        val directId: Long? = when (val v = extras?.get(EXTRA_CLONED_APP_ID)) {
            is Long -> v
            is Int -> v.toLong()
            is Number -> v.toLong()
        ...
        // Fallback to package-based lookup
        val clonedPackage = extras?.getString(EXTRA_CLONED_PACKAGE)
            ?: intent.getStringExtra(EXTRA_CLONED_PACKAGE)
        if (clonedPackage.isNullOrBlank()) return null

        return try {
            databaseHelper.getClonedAppIdByPackageName(clonedPackage)
        } catch (e: Exception) {
            Log.e(TAG, "DB lookup failed for cloned package: $clonedPackage", e)
            null
        }
    }

    private fun performCloneRemoval(clonedAppId: Long) {
        try {
            Log.d(TAG, "Attempting to remove cloned app id=$clonedAppId via intent")
            val success = virtualSpaceEngine.removeVirtualSpace(clonedAppId)
            if (success) {
                Log.d(TAG, "✅ Clone removed successfully (id=$clonedAppId)")
            } else {
                Log.w(TAG, "⚠️ Clone removal returned false (id=$clonedAppId)")
            }
        } catch (e: Exception) {
            Log.e(TAG, "❌ Error removing clone id=$clonedAppId", e)
        }
    }
    ...
    override fun configureFlutterEngine(@NonNull flutterEngine: FlutterEngine) {
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, CHANNEL).setMethodCallHandler { call, result ->
            ...
            val clonedAppId = when (val idArg = call.argument<Any>("id")) {
                ...
                // New: Global WebView/Cookie clear (safe, main-thread)
                "clearGlobalWebArtifacts" -> {
                    fallbackClearGlobalWebArtifacts()
                    result.success(true)
                }
                // New: Clear per-clone web/auth data by clonedAppId (Long)
                "clearCloneWebArtifactsById" -> {
                    val idArg = call.argument<Any>("clonedAppId")
                    val clonedAppId = when (idArg) {
                        is Long -> idArg
                        is Int -> idArg.toLong()
                        is Number -> idArg.toLong()
                    ...
                    if (clonedAppId == null) {
                        result.error("INVALID_ARGUMENT", "clonedAppId is required", null)
                    } else {
                        try {
                            val ok = virtualSpaceEngine.clearCloneWebArtifactsById(clonedAppId)
                            result.success(ok)
                        } catch (e: Exception) {
                            Log.e(TAG, "Failed to clear clone web artifacts for id=$clonedAppId", e)
                            result.error("CLEAR_ERROR", e.message, null)
                        }
                    }
                }
                // New: Enforce fresh login for a clone by ID
                "enforceFreshLoginById" -> {
                    val idArg = call.argument<Any>("clonedAppId")
                    val clonedAppId = when (idArg) {
                        is Long -> idArg
                        is Int -> idArg.toLong()
                    ...
                    if (clonedAppId == null) {
                        ...
                        try {
                            val ok = virtualSpaceEngine.enforceFreshLoginById(clonedAppId)
                            result.success(ok)
                        } catch (e: Exception) {
                            Log.e(TAG, "Failed to enforce fresh login for id=$clonedAppId", e)
                            result.error("ENFORCE_ERROR", e.message, null)
                        }
                    }
                }
                else -> {
    ...
    private fun getInstalledApps(
        ...
        backgroundExecutor.execute {
            try {
                ...
                // On Android 11+ QUERY_ALL_PACKAGES cannot be requested at runtime; rely on <queries> visibility
                val hasQueryPermission = true
                Log.d(TAG, "Proceeding without runtime QUERY_ALL_PACKAGES request; relying on <queries> visibility")
                // Continue without attempting a runtime request
                ...
                var packages = try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            mainHandler.post {
                                result.error("PERMISSION_ERROR", "Unable to access installed apps. Please ensure app visibility via <queries> in manifest.", null)
                            }
                            return@execute
                        }
                    }
                ...
                // If list is still empty, fallback to resolving LAUNCHER activities
                if (packages.isEmpty()) {
                    try {
                        Log.w(TAG, "Package list empty; falling back to LAUNCHER resolve infos")
                        ...
                        for (ri in resolveInfos) {
                            try {
                                ...
                                fallbackPackages.add(pi)
                            } catch (ignored: Exception) { }
                        }
                        Log.d(TAG, "Fallback via queryIntentActivities found ${fallbackPackages.size} apps")
                        packages = fallbackPackages
                    } catch (e: Exception) {
                        Log.w(TAG, "Fallback queryIntentActivities failed: ${e.message}")
                    }
                }
                ...
                if (appsList.isEmpty()) {
                    Log.w(TAG, "WARNING: No apps found! This might indicate a visibility or filtering issue")
                    Log.w(TAG, "Debug info - excludeSystemApps=$excludeSystemApps, hasQueryPermission=$hasQueryPermission")
                    // Do not treat as hard error; return empty list so UI can retry without a red error bar
                }
    ...
    private fun launchClonedApp(call: MethodCall, result: MethodChannel.Result) {
        try {
            ...
            val launchSuccess = if (clonedAppId != null) {
                ...
                fallbackMaybeEnforceFreshForLegacy(packageName, virtualSpaceId)
                val hookingSystem = HookingSystem(this)
    ...
    // Fallback global web artifacts clearing for legacy launch path
    private fun fallbackClearGlobalWebArtifacts() {
        try {
            ...
            val task = Runnable {
                try {
                    ...
                    Log.d(TAG, "Fallback cleared cookies and web storage before legacy launch")
                } catch (inner: Throwable) {
                    Log.w(TAG, "Fallback web clear failed: ${inner.message}")
                } finally {
                    latch.countDown()
                }
            }
            if (Looper.myLooper() == mainLooper) task.run() else Handler(mainLooper).post(task)
            // Wait briefly to ensure the clear executes before launch
            latch.await(2, java.util.concurrent.TimeUnit.SECONDS)
        } catch (e: Throwable) {
            Log.w(TAG, "Failed to schedule fallback web clear: ${e.message}")
        }
    }

    // One-time enforcement: clear only before first launch of a legacy clone
    private fun fallbackMaybeEnforceFreshForLegacy(packageName: String, virtualSpaceId: String) {
        try {
            val prefs = getSharedPreferences("legacy_clone_enforce_fresh", Context.MODE_PRIVATE)
            val key = "enforced_${packageName}_${virtualSpaceId}"
            if (!prefs.getBoolean(key, false)) {
                Log.d(TAG, "Enforcing fresh sign-in for legacy clone $packageName/$virtualSpaceId")
                fallbackClearGlobalWebArtifacts()
                prefs.edit().putBoolean(key, true).apply()
            } else {
                Log.d(TAG, "Fresh sign-in already enforced for legacy clone $packageName/$virtualSpaceId")
            }
        } catch (e: Exception) {
            Log.w(TAG, "Failed to enforce fresh for legacy clone: ${e.message}")
        }
    }
    ...
    private fun stopClonedApp(packageName: String, virtualSpaceId: String) {
        try {
            ...
            if (ContextCompat.checkSelfPermission(this, Manifest.permission.KILL_BACKGROUND_PROCESSES) == PackageManager.PERMISSION_GRANTED) {
                ...
                Log.w(TAG, "KILL_BACKGROUND_PROCESSES not granted; skipping killBackgroundProcesses for ${packageName}_${virtualSpaceId}")
            }
        } catch (e: Exception) {
            // Handle exception
        }
            ...
            when (permission) {
                ...
                "android.permission.SYSTEM_ALERT_WINDOW" -> {
                    val hasPermission = if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
                        ...
                        // Before API 23, SYSTEM_ALERT_WINDOW was granted at install time
                        true
                    }
                    result.success(hasPermission)
                }
